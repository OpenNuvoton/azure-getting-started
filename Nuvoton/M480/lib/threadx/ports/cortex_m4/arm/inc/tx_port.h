...
static __forceinline ULONG __get_control(void)
{
    register ULONG __regControl             __asm("control");
    return(__regControl);
}


static __forceinline void __set_control(ULONG control_value)
{
    register ULONG __regControl             __asm("control");
    __regControl = control_value;
}
...
static __forceinline unsigned int __get_ipsr_value(void)
{
...
__asm static __forceinline ULONG TX_LOWEST_SET_BIT_CALCULATE_(ULONG m)
{
    RBIT    R0, R0
    CLZ     R0, R0
    BX      LR
...
#define TX_LOWEST_SET_BIT_CALCULATE(m, b)       \
    b = TX_LOWEST_SET_BIT_CALCULATE_(m);
...
/* Define ARMCC specific macros, with in-line assembly for performance.  */

__asm static __forceinline unsigned int __disable_interrupts(void)
{
    MRS     R0, PRIMASK
    CPSID   i
    BX      LR
...
static __forceinline void __restore_interrupts(unsigned int primask_value)
{
    register unsigned int __regPriMask          __asm("primask");
  __regPriMask = (primask_value);
}

static __forceinline unsigned int __get_primask_value(void)
{
    register unsigned int __regPriMask          __asm("primask");
    return(__regPriMask);
}

static __forceinline void __enable_interrupts(void)
{
    __enable_irq();
}


static __forceinline void _tx_thread_system_return_inline(void)
{
